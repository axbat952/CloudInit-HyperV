...
param(
  ...
  [string] $VMVersion = "10.0", # version 8.0 for hyper-v 2016 compatibility , check all possible values with Get-VMHostSupportedVersion
  ...
  [string] $KeyboardLayout = "fr", # 2-letter country code, for more info https://wiki.archlinux.org/title/Xorg/Keyboard_configuration
  [string] $KeyboardModel, # default: "pc105"
  [string] $KeyboardOptions, # example: "compose:rwin"
  [string] $Locale = "fr_FR", # "en_US.UTF-8",
  [string] $TimeZone = "Europe/Paris", # UTC or continental zones of IANA DB like: Europe/Berlin
  ...
  [string] $GuestAdminPassword = "Password",
...
Start-VM $VMName
Write-Host -ForegroundColor Green " Done."
get-vm -Name $VMName | select -ExpandProperty networkadapters | select vmname, macaddress, switchname, ipaddresses | ft -wrap -autosize
